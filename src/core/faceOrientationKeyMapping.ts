...
export type CameraFace = "front" | "back" | "top" | "bottom" | "right" | "left";
...
const BASE_ORIENTATIONS: Record<CameraFace, { look: THREE.Vector3; up: THREE.Vector3 }> = {
  front:  { look: new THREE.Vector3(0, 0, -1), up: new THREE.Vector3(0, 1, 0) },
  back:   { look: new THREE.Vector3(0, 0, 1),  up: new THREE.Vector3(0, 1, 0) },
  top:    { look: new THREE.Vector3(0, -1, 0), up: new THREE.Vector3(0, 0, 1) },
  bottom: { look: new THREE.Vector3(0, 1, 0),  up: new THREE.Vector3(0, 0, -1) },
  right:  { look: new THREE.Vector3(-1, 0, 0), up: new THREE.Vector3(0, 1, 0) },
  left:   { look: new THREE.Vector3(1, 0, 0),  up: new THREE.Vector3(0, 1, 0) }
};

function getLocalQuaternion(face: CameraFace, rotation: CameraRotation): THREE.Quaternion {
  const { look, up } = BASE_ORIENTATIONS[face];
  const m = new THREE.Matrix4().lookAt(new THREE.Vector3(0, 0, 0), look, up);
  const q = new THREE.Quaternion().setFromRotationMatrix(m);
  const roll = new THREE.Quaternion().setFromAxisAngle(new THREE.Vector3(0, 0, 1), THREE.MathUtils.degToRad(-rotation));
...
export function getRotationAxis(face: CameraFace, rotation: CameraRotation, type: 'horizontal' | 'vertical' | 'roll'): THREE.Vector3 {
  ...
  switch (type) {
    case 'vertical': localVector = new THREE.Vector3(1, 0, 0); break; // Local +X maps to 'd' in old logic
    case 'horizontal': localVector = new THREE.Vector3(0, 1, 0); break; // Local +Y maps to 'w' in old logic
    case 'roll': localVector = new THREE.Vector3(0, 0, -1); break; // Local -Z maps to 'q' in old logic
  }
...
export function getExplicitRotationAxis(face: CameraFace, rotation: CameraRotation, key: 'o' | 'k' | 'period' | 'semicolon' | 'i' | 'p'): THREE.Vector3 {
  ...
  switch (key) {
    case 'o': localVector.set(1, 0, 0); break;
    case 'period': localVector.set(-1, 0, 0); break;
    case 'semicolon': localVector.set(0, -1, 0); break;
    case 'k': localVector.set(0, 1, 0); break;
    case 'i': localVector.set(0, 0, 1); break;
    case 'p': localVector.set(0, 0, -1); break;
  }
 ...
 */
export function getWASDMapping(face: CameraFace, rotation: CameraRotation): Record<string, number[]> {
  const q = getLocalQuaternion(face, rotation);
  return {

...
export class Grid3D extends Emitter<{ tick: undefined }> {
	// Extend Emitter
	size: number;
	...
	tick(
		...
		const communityMap =
			birthMargin > 0 ? this.getAllCommunities() : null;
		const key = (x: number, y: number, z: number) => `${x},${y},${z}`;
							...
							for (
								...
								for (
									...
									for (
										...
										cx <=
										Math.min(this.size - 1, Math.ceil(x + margin));
										cx++
									) {
		...
		for (let x = 0; x < 3; x++) {
			for (let y = 0; y < 3; y++) {
				for (let z = 0; z < 3; z++) {
					...
					const edgeCount = [x === 1, y === 1, z === 1].filter(
						Boolean,
					).length;
					if (edgeCount < 2) {
						cells.push([x, y, z]);

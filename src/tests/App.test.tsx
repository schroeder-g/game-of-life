...
describe('Automated UI & Feature Tests', () => {
	...
	describe('[UX-5, UI-1] Build/Environment Specific UI', () => {
		...
		it('[UI-1] hides Manual Tests panel in a prod build', () => {
			...
			expect(
				screen.queryByText('Manual Tests'),
			).not.toBeInTheDocument();
		});
	});
	...
	describe('[UX-6] Test Panel Persistence', () => {
		it('[UX-6] saves and loads test statuses from localStorage', async () => {
			...
			const firstTestTitle = await screen.findByText(
				MANUAL_TESTS[0].title,
			);
			// The status icon div is the sibling before the title's parent
			const testRow =
				firstTestTitle.closest('div[style*="display"]') ||
				firstTestTitle.parentElement;
			const statusIconDiv =
				testRow?.querySelector('div') ||
				testRow?.previousElementSibling;
			if (statusIconDiv) {
			...
			expect(parsed[firstId]?.status ?? parsed[firstId]).toBe(
				'checked',
			);
		});
	});

...
} from '../data/documentation/_Documentation';
...
export function DocumentationModal({
	...
	const {
		claimStatuses: automatedClaimStatuses,
		isLoading: isLoadingAutomatedTests,
		error: automatedTestsError,
		testDate: automatedReportDate,
	} = useAutomatedTestResults(); // Use the new hook
	...
	const currentItems = DOCUMENTATION_CONTENT.filter(
		item =>
			!item.id.startsWith('deprecated-') &&
			item.id !== 'heading-deprecated',
	);
	const deprecatedItems = DOCUMENTATION_CONTENT.filter(
		item =>
			item.id.startsWith('deprecated-') ||
			item.id === 'heading-deprecated',
	);
	const deprecatedHeader = deprecatedItems.find(
		item => item.id === 'heading-deprecated',
	);
	const deprecatedClaims = deprecatedItems.filter(
		item => item.id !== 'heading-deprecated',
	);
	...
	const generateGroupedIndex = (): GroupedIndexSection[] => {
		...
		DOCUMENTATION_INDEX_GROUPS.forEach(group => {
			...
			const relatedItems = DOCUMENTATION_CONTENT.filter(
				item =>
					item.type === 'h3' &&
			...
			relatedItems.forEach(item => {
				...
				const cleanedText = stripHtmlTags(
					item.text.replace(group.stripPrefix, ''),
				);
							...
							<div style={{ marginTop: '0.5rem' }}>
								{item.testIds.map((testId: string) => {
									...
									const automatedStatus =
										automatedClaimStatuses.get(testId);
									...
									let testType: 'Manual' | 'Automated' | 'Untested' =
										'Untested';
...
function IndexModal({
	...
	return createPortal(
		<div
			className='modal-overlay index-overlay'
			onClick={onClose}

...
vi.mock('../../hooks/usePersistentState', () => ({
	usePersistentState: (key: string, defaultValue: any) => [
		defaultValue,
		vi.fn(),
	],
}));
...
describe('SettingsSidebar', () => {
	...
	beforeEach(() => {
		...
		mockGenesisConfigState = {
			savedConfigs: {
				Default: { name: 'Default', cells: [], settings: {} },
			},
		...
		mockUseSimulation.mockReturnValue({
			...
			meta: {
				...
				cameraActionsRef: {
					current: {
						birthBrushCells: vi.fn(),
						clearBrushCells: vi.fn(),
					},
				},
			},
		});
	...
	it('[SS_SCENE_NAME_INPUT_001_TEST][UX-7] focused input suppresses global shortcuts', async () => {
		...
		if (
			sceneManagementHeader.nextElementSibling?.classList.contains(
				'collapsed',
			)
		) {
		...
		const sceneNameInput = screen.getByPlaceholderText(
			'New scene name...',
		);
	...
	it('[SS_CURSOR_POSITION_KEY_HINTS_001_TEST][QA-2] SelectorPositionSection displays correct key hints based on camera orientation', () => {
		...
		const selectorPositionHeader = screen.getByRole('heading', {
			name: /Cursor Position/i,
		});
		// If it's collapsed, click to expand.
		if (
			selectorPositionHeader.nextElementSibling?.classList.contains(
				'collapsed',
			)
		) {
		...
		// Check X axis hints
		const xCoordContainerRight = screen
			.getByText('X')
			.closest('.coordinate-input-container');
		...
		// Check Y axis hints
		const yCoordContainerRight = screen
			.getByText('Y')
			.closest('.coordinate-input-container');
		...
		// Check Z axis hints
		const zCoordContainerRight = screen
			.getByText('Z')
			.closest('.coordinate-input-container');

...
describe('Grid3D', () => {
  ...
  it('[CORE-1] should initialize with correct size and empty cells', () => {
    expect(grid.size).toBe(10);
    expect(grid.get(0, 0, 0)).toBe(false);
    expect(grid.get(9, 9, 9)).toBe(false);
  });

  it('[CORE-2] should not throw on out of bounds get', () => {
    expect(grid.get(-1, 0, 0)).toBe(false);
    expect(grid.get(10, 10, 10)).toBe(false);
  });

  it('[CORE-3] should set and get cell states correctly', () => {
    grid.set(1, 2, 3, true);
    expect(grid.get(1, 2, 3)).toBe(true);
    expect(grid.get(0, 0, 0)).toBe(false); 
  });

  it('[CORE-4] should ignore out of bounds set', () => {
    grid.set(-1, 0, 0, true);
    expect(grid.get(-1, 0, 0)).toBe(false);
  });

  it('[CORE-5] should toggle cell states correctly', () => {
    grid.toggle(5, 5, 5);
  ...
  it('[CORE-6] should clear all cells', () => {
    grid.set(1, 1, 1, true);
    grid.set(2, 2, 2, true);
    grid.clear();
  ...
  it('[CORE-7] should save and restore state', () => {
    grid.set(1, 1, 1, true);
    grid.set(2, 2, 2, true);
  ...
  it('[CORE-8] should simulate a tick correctly (survival)', () => {

...
inline bool getBB(const vector<Cell>& cells, int& min_x, int& min_y, int& min_z) {
    ...
    for (size_t i = 1; i < cells.size(); ++i) {
        ...
        min_z = min(min_z, cells[i].z);
    }
    return true;
}

// Creates a canonical string representation of a cell pattern.
inline string norm(const vector<Cell>& cells) {
    ...
    string res = "";
    for (size_t i = 0; i < n.size(); ++i) {
        res += to_string(n[i].x) + "," + to_string(n[i].y) + "," + to_string(n[i].z) + "|";
    }
    return res;
}

// Extracts all living cells from a given grid.
inline vector<Cell> getCells(const unsigned char* grid, int size) {
    vector<Cell> out;

...
import { DOCUMENTATION_CONTENT, DOCUMENTATION_INDEX_GROUPS } from '../src/data/documentation/_Documentation';
...
DOCUMENTATION_INDEX_GROUPS.forEach(group => {
  const groupItems = DOCUMENTATION_CONTENT.filter(item => 
    item.id.startsWith(group.idPrefix) && !item.id.startsWith('deprecated-') && item.type === 'h3'
  );
...
DOCUMENTATION_CONTENT.forEach(item => {
  if (!item.id.startsWith('deprecated-') && !allGroupedIds.has(item.id) && item.type === 'h3') {
